...
-- Rule 4: Admin can see all students (via email check for admin@sgu.edu.in)
...
create policy "Admin can read all students" on students for select
  using (auth.jwt()->>'email' = 'admin@sgu.edu.in');
...
create policy "Admin can create jobs" on jobs for insert
  with check (auth.jwt()->>'email' = 'admin@sgu.edu.in');
...
-- ============================================================================
-- Rule 1: Users can read messages in their conversations
-- (This is simplified - in production you'd check the conversation ownership)
drop policy if exists "Users can read messages" on messages;
create policy "Users can read messages" on messages for select
  using (auth.role() = 'authenticated');

-- Rule 2: Authenticated users can send messages
